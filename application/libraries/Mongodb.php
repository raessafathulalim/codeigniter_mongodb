...
class Mongodb{
	...
	private function prepare(){
		...
		if(isset($this->config[$this->activate]) == TRUE){
			if(empty($this->config[$this->activate]['dns'])){
				show_error("DNS missing from mongodb config group : {$this->activate}", 500);
			}else{
				$this->dns = trim($this->config[$this->activate]['dns']);
			}
	...
	private function connect(){
		$this->prepare();
		try{
			$this->connect = new MongoDB\Client($this->dns);
	...
	* @return : objectid : if have insert _id return _id, if not set _id will return id.
	*/
	public function insert($collection = "", $insert = array()){
		...
		try{
			...
			$query = $table->insertOne($insert, array('w' => $this->write_concerns, 'j'=>$this->journal));
			if (isset($insert['_id'])){
				return ($insert['_id']);
			}else{
	...
	*
	* @usage : $this->mongodb->batch_insert('foo', $data = array());
	* @return : array : if have insert _id return _id, if not set _id will return id.
	*/
	public function batch_insert($collection = "", $insert = array()){
		...
		try{
			$table = $this->db->selectCollection($collection);
			$query = $table->insertMany($insert, array('w' => $this->write_concerns, 'j'=>$this->journal));
			if(is_array($insert) && count($insert) > 0){
				$insert_ids = array_map(function ($arr) {return $arr['_id'];},$insert);
				return ($insert_ids);
			}else {
				return $query->getInsertedIds();
			}
		}catch (MongoCursorException $e){
			if(isset($this->debug) == TRUE && $this->debug == TRUE){
	...
	*
	* @usage : $this->mongodb->or_where(array('foo'=>'bar', 'bar'=>'foo'))->get('foobar');
	*/
	...
	*
	* @usage : $this->mongodb->where_in('foo', array('bar', 'zoo', 'blah'))->get('foobar');
	*/
	...
	*
	* @usage : $this->mongodb->where_in_all('foo', array('bar', 'zoo', 'blah'))->get('foobar');
	*/
	...
	*
	* @usage : $this->mongodb->where_not_in('foo', array('bar', 'zoo', 'blah'))->get('foobar');
	*/
	...
	*
	* @usage : $this->mongodb->where_gt('foo', 20);
	*/
	...
	*
	* @usage : $this->mongodb->where_gte('foo', 20);
	*/
	...
	*
	* @usage : $this->mongodb->where_lt('foo', 20);
	*/
	...
	*
	* @usage : $this->mongodb->where_lte('foo', 20);
	*/
	...
	*
	* @usage : $this->mongodb->where_between('foo', 20, 30);
	*/
	...
	*
	* @usage : $this->mongodb->where_between_ne('foo', 20, 30);
	*/
	...
	*
	* @usage : $this->mongodb->where_ne('foo', 1)->get('foobar');
	*/
	...
	*/
	public function get($collection = ""){			
		...
		try{
			...
			if(isset($this->selects) && is_array($this->selects)){
				$options['projection']=$this->selects;
			}
			$results = $documents->find($this->wheres, $options);

			// $this->explain($documents, $results);
			// Clear
			$this->_clear();
			$returns = array();
	...
	* Get the documents based upon the passed parameters
	*
	* @usage : $this->mongodb->get_where('foo', array('bar' => 'something'));
	*/
	...
	*
	* @usage : $this->mongodb->find_one('foo');
	*/
	public function find_one($collection = ""){
		...
		try{
			...
			$document = $this->db->{$collection}->findOne($this->wheres, $options);
			// Clear
			$this->_clear();
			if(is_null($document)){
				return false;
	...
	*/
	public function count($collection = "") {
		...
		$count = $this->db->{$collection}->count($this->wheres, $options);
		$this->_clear();
		return ($count);
	}
	...
	*
	* @usage: $this->mongodb->where(array('blog_id'=>123))->set('posted', 1)->update('blog_posts');
	* @usage: $this->mongodb->where(array('blog_id'=>123))->set(array('posted' => 1, 'time' => time()))->update('blog_posts');
	*/
	...
	*
	* @usage: $this->mongodb->where(array('blog_id'=>123))->unset('posted')->update('blog_posts');
	* @usage: $this->mongodb->where(array('blog_id'=>123))->set(array('posted','time'))->update('blog_posts');
	*/
	...
	*
	* @usage: $this->mongodb->where(array('blog_id'=>123))->addtoset('tags', 'php')->update('blog_posts');
	* @usage: $this->mongodb->where(array('blog_id'=>123))->addtoset('tags', array('php', 'codeigniter', 'mongodb'))->update('blog_posts');
	*/
	...
	*
	* @usage: $this->mongodb->where(array('blog_id'=>123))->push('comments', array('text'=>'Hello world'))->update('blog_posts');
	* @usage: $this->mongodb->where(array('blog_id'=>123))->push(array('comments' => array('text'=>'Hello world')), 'viewed_by' => array('Alex')->update('blog_posts');
	*/
	...
	*
	* @usage: $this->mongodb->where(array('blog_id'=>123))->pop('comments')->update('blog_posts');
	* @usage: $this->mongodb->where(array('blog_id'=>123))->pop(array('comments', 'viewed_by'))->update('blog_posts');
	*/
	...
	*
	* @usage: $this->mongodb->pull('comments', array('comment_id'=>123))->update('blog_posts');
	*/
	...
	* Renames a field
	*
	* @usage: $this->mongodb->where(array('blog_id'=>123))->rename_field('posted_by', 'author')->update('blog_posts');
	*/
	...
	*
	* @usage: $this->mongodb->where(array('blog_id'=>123))->inc(array('num_comments' => 1))->update('blog_posts');
	*/
	...
	*
	* @usage: $this->mongodb->where(array('blog_id'=>123))->mul(array('num_comments' => 3))->update('blog_posts');
	*/
	...
	*
	* @usage: $this->mongodb->where(array('blog_id'=>123))->max(array('num_comments' => 3))->update('blog_posts');
	*/
	...
	*
	* @usage: $this->mongodb->where(array('blog_id'=>123))->min(array('num_comments' => 3))->update('blog_posts');
	*/
	...
	*
	* @usage: $this->mongodb->update_all('foo', $data = array());
	*/
	public function update_all($collection = "", $data = array(), $options = array()){
		...
		try{
			...
			$this->db->{$collection}->updateMany($this->wheres, $this->updates, $options);
			$this->_clear();
			return (TRUE);
		}catch (MongoCursorException $e){
			if(isset($this->debug) == TRUE && $this->debug == TRUE){
	...
	*
	* @usage : $this->mongodb->delete_all('foo');
	*/
	...
	* set to 1 (ASC).
	*
	* @usage : $this->mongodb->order_by(array('foo' => 'ASC'))->get('foobar');
	*/
	...
	*
	* @usage : $this->mongodb->output_benchmark();
	*/
	...
	* set to 1 (ASC).
	*
	* @usage : $this->mongodb->add_index($collection, array('first_name' => 'ASC', 'last_name' => -1), array('unique' => TRUE));
	*/
	public function add_index($collection = "", $keys = array(), $options = array()){
		...
		try{
			$this->db->{$collection}->createIndex($data_keys, $options);
			$this->_clear();
			return TRUE;
		}catch (MongoCursorException $e){
			if(isset($this->debug) == TRUE && $this->debug == TRUE){
	...
	* set to 1 (ASC).
	*
	* @usage : $this->mongodb->remove_index($collection, array('first_name' => 'ASC', 'last_name' => -1));
	*/
	public function remove_index($collection = "", $keys = array()){
		...
		try{
			foreach ($keys as $key => $value) {
				$this->db->{$collection}->dropIndex($value);
			}
			$this->_clear();
			return TRUE;
		}catch (MongoCursorException $e){
	...
	*
	* @usage : $this->mongodb->list_indexes($collection);
	*/
	public function list_indexes($collection = ""){
		if (empty($collection)){
			show_error("No Mongo collection specified to see list all indexes", 500);
		}
		return ($this->db->{$collection}->listIndexes());
	}
	...
	* @usage: $this->mongodb->drop_db("foobar");
	*/
	...
	* @usage: $this->mongodb->drop_collection('bar');
	*/
	...
	*/
	...
	private function explain($cursor, $results, $aggregate=null){
		array_push($this->benchmark, 
			array(
				'benchmark'=>$cursor->explain($results),
				'query'=> array(
